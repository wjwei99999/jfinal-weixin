...
public class ComponentAccessTokenApi {
    ...
    public static synchronized void refreshComponentAccessToken() {
        ...
        final Map<String, String> queryParas = ParaMap
            .create("component_appid", appId)
            .put("component_appsecret", appSecret)
            .put("component_verify_ticket", ticket)
            .getData();

        // 最多三次请求
        ComponentAccessToken result = RetryUtils
            .retryOnException(3, new Callable<ComponentAccessToken>() {
                @Override
                public ComponentAccessToken call() throws Exception {
                    String json = HttpUtils.post(url, JsonUtils.toJson(queryParas));
                    return new ComponentAccessToken(json);
                }
            });

        // 三次请求如果仍然返回了不可用的 access token 仍然 put 进去，便于上层通过 AccessToken 中的属性判断底层的情况
        tokenCache.setComponentAccessToken(ac.getAppId(), result);
    }
